...
export type StartResult = { ok: true; endsAt: number } | { ok: false; message: string };
...
export const ACTION_STARTERS: Record<EHexActionType, ActionStarter> = {
  [EHexActionType.CUT]: (tile, tool, now, ctx) => {
    ...
    const heroToolStore = ctx.heroToolStore;
  ...
  [EHexActionType.MINE]: (tile, tool, now, ctx) => {
    const obj = tile.hexobject;
    if (!obj) return { ok: false, message: 'Hex has no object!' };
  ...
  [EHexActionType.TAKE]: (tile, tool, now, ctx) => {
    const obj = tile.hexobject;
  ...
  [EHexActionType.USE]: (tile, tool, now, ctx) => {
    const obj = tile.hexobject;
    if (!obj) return { ok: false, message: 'Hex has no object!' };
    if (isBusy(tile, now)) return { ok: false, message: 'Tile is busy!' };
  ...
  [EHexActionType.ATTACK]: (tile, _tool, now, ctx) => {
    if (isBusy(tile, now)) return { ok: false, message: 'Hex is busy!' };
    const res = ctx.worldMap.performHeroCombatAttack(tile, _tool);
    if (!res.ok) return { ok: false, message: res.message };
    return { ok: true, endsAt: now };
  },

  [EHexActionType.BLOCK]: (tile, _tool, now, ctx) => {
    if (isBusy(tile, now)) return { ok: false, message: 'Hex is busy!' };
    const placed = ctx.worldMap.placeCombatDefendMarker(tile.coordinates, _tool);
  ...
  [EHexActionType.ENTER]: (tile, tool, now, ctx) => {
    const heroToolStore = ctx.heroToolStore;
    const heroStore = ctx.hero;
    const gameEventsStore = ctx.events;
    const worldStore = ctx.worldMap;

    if (isBusy(tile, now)) return { ok: false, message: 'Hex is busy!' };
    if (worldStore.combatActive)
      return { ok: false, message: 'Cannot leave the map during combat!' };
    ...
    if (meta?.enter?.type === 'WORLD') {
      if (worldStore.isLocationRespawning(meta.enter.locationKey)) {
        const remainingSeconds = Math.ceil(
          worldStore.getLocationRespawnRemainingMs(meta.enter.locationKey) / 1000,
        );
      ...
      ctx.navigate?.(meta.enter.locationKey);
    }
