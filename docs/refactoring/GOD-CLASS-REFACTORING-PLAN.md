...
## Частина 6 — Цільовий контракт `world-map-store` (написано 10.08.2026, після G0–G6)

### 6.0. Навіщо цей розділ

Підсумок G0–G6: рядків стало менше (914 → 612), **обов'язків — ні** (32 → 35 actions). Розпил дав Extract Function там, де потрібен був Extract Class. Причина не в якості виконання фаз — вони зроблені акуратно. Причина в тому, що **ніде не було записано, чим у цій архітектурі є Pinia-стор**. Без цього означення кожен наступний розпил приземляється назад у стор — що вже й сталося: `hero-store` виріс 157 → 272 і забрав рух героя разом із власним `localStorage`.

Спостереження, яке варто зафіксувати окремо: **розпил спрацював рівно там, де перетнув межу пакета.** Усе, що поїхало в `packages/engine/` (`fog-service`, `move-planner`, `free-hex-finder`, `combat/*`), — справжній Extract Class із власними тестами й нулем зворотних посилань. Усе, що поїхало в `apps/web/src/services/`, лишилось зчепленим зі стором. Межа папки нічого не змушує. Межа пакета змушує.

Цей розділ — **не наступна фаза**. Це критерій приймання для будь-якої майбутньої зміни: «чи не кладе цей код правило в стор?».

### 6.1. Чим є стор у нашій архітектурі

Наш конвеєр (`applyCommand` → доменні події) — це, по суті, CQRS. У такій архітектурі клієнтський стор має рівно дві ролі:

> **Read model (проєкція) + воронка намірів.**

Стор не є доменним об'єктом. Він не містить правил. Він відповідає на «що UI має намалювати зараз» і приймає «користувач хоче X». Усе інше — чуже.

**Формулювання єдиного обов'язку:**

> `world-map-store` тримає зріз світу, який зараз бачить цей клієнт, і є єдиною точкою, через яку наміри щодо світу потрапляють у рушій, а події з рушія — в UI.

**Єдина причина для зміни:** змінилось те, що UI мапи має _бачити_ або _надсилати_.

Не причина для зміни: змінився формат збереження, правило руху, правило туману, дизайн бою, спосіб навігації. Якщо будь-що з цього змушує редагувати стор — контракт порушено.

### 6.2. Що стор ТРИМАЄ

```ts
{
  map: HexMapModel | null; // зріз, яким володіє, але не який авторує
  locationKey: LocationKey;
  mapId: string | null;
  status: 'idle' | 'loading' | 'ready' | 'error';
  revision: number; // += 1 на кожну застосовану пачку подій
}
```

Два зауваження, що не є косметикою:

- **`status` замість `map === null`.** Зараз `null` одночасно означає «ще не вантажили», «вантажимо», «помилка» і «для цієї локації мапи немає». Це джерело цілого класу гонок, які зараз ловляться руками через `if (!this.map) return` у 15 місцях.
- **`revision` замість `dirtyTick`.** Див. 6.3.

### 6.3. Що стор НЕ тримає (і чому саме)

| Зараз тримає                          | Куди                                | Чому                                                                                                                                                                                                                              |
| ------------------------------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dirtyTileIds` / `dirtyTick`          | видалити                            | Рендеру потрібні дельти — а конвеєр уже їх виробляє у вигляді доменних подій. `markTileDirty()` викликається вручну в ~8 місцях; це 8 шансів забути. Подія `HERO_MOVED` уже точно знає, які тайли змінились.                      |
| респавн локацій                       | session/campaign-шар                | Це стан кампанії, а не поточної мапи. Він переживає зміну мапи — отже, не належить мапі.                                                                                                                                          |
| `heroCoordinates` (вже виїхав у G4.5) | `heroes: Record<HeroId, HeroState>` | Напрямок був правильний, адреса — ні. Однина `hero-store.heroCoordinates` — це single-player-артефакт. `MOVE_HERO` у рушії вже приймає `heroes: Record<id, HeroState>`. Це наступне латентне припущення про однокористувацькість. |
| combat / inventory / hero-стан        | свої стори                          | —                                                                                                                                                                                                                                 |

### 6.4. Що стор ВМІЄ — чотири методи замість тридцяти п'яти

```ts
open(locationKey, preferredMapId?): Promise<void>   // приєднати зріз через WorldRepository
dispatch(command: HexEngineCommand): DomainEvent[]  // єдина воронка намірів
applyRemote(events: DomainEvent[]): void            // події з сервера / реплею — той самий шлях
close(): void                                        // від'єднати
```

плюс **гетери** (не actions): `tileAt(coords)`, `isReady`, `revision`.

Дрібниця, що симптоматична: `getTileAt` сьогодні — action. Це запит, а не команда. Коли запити оформлені як actions, зникає межа між «прочитати» і «змінити».

Головне тут — **`dispatch` як єдина воронка**. Це і є виграш за OCP: додавання `PLAY_CARD`, `ROLL_DICE`, `USE_ABILITY` (тобто вся Частина 2 `GAMEPLAY-VERTICAL-SLICE-PLAN.md`) **не торкається стора взагалі**. Сьогодні кожна нова механіка = новий action у сторі, і саме тому їх 35.

`applyRemote` — не «на майбутнє». Це те, що робить drop-in/drop-out можливим у принципі: локальна дія й дія іншого гравця мають проходити однаковим кодом, інакше клієнти розійдуться.

### 6.5. Про що стор ЗНАЄ — білий список із трьох пунктів

1. Типи рушія + `applyCommand`.
2. Порт `WorldRepository` (інтерфейс) — `load`/`save`/`forget`.
3. Стік подій (шина).

**Не знає:** `localStorage`, `router`, `setInterval`, жодного іншого Pinia-стора, і не знає слів «туман», «бій», «інвентар».

Порт `WorldRepository` — це те, що робить один і той самий стор придатним і для single-player (реалізація через `localStorage`), і для co-op (реалізація через API). LSP тут не абстракція заради абстракції: якщо дві реалізації не взаємозамінні, спільної кодової бази для одиночної гри й кооперативу не буде.

### 6.6. Дві речі, які варто назвати окремо

**`buildEngineContext()` — ось де насправді живе God Class.**

Він збирає 5 сторів плюс порт-адаптер над `combat-store` і над самим собою. Це найщільніша точка зчеплення в усьому `apps/web` і безпосередня причина більшості з 11 циклів у `KNOWN_CYCLE_FILES`. Стор фізично не може роз'єднатися, поки він одночасно є DI-контейнером. У коді вже висить коментар, що це борг (`MIGRATION-PLAN.md`, фаза 2) — але борг цей тепер блокуючий, а не косметичний.

Лікування: контекст збирається **один раз у composition root** (окремий модуль на старті застосунку) і **інжектується**, а не складається всередині стора на кожен виклик. Це одна зміна, після якої більшість циклів зникає сама — без жодного нового розпилу.

**Хто крутить світовий цикл.**

`world-loop.ts` виділений структурно правильно, але **політику досі тримає стор** (`startWorldLoop`/`stopWorldLoop`). У кооперативі тікає _сервер_. Якщо кожен клієнт тікає власний світ, вони розходяться за побудовою — це не баг, який колись виявиться, а гарантований наслідок. Володіння відповіддю «коли тече час» має переїхати в session/scenario-шар, який можна підмінити на «нам сказав сервер».

### 6.7. Куди дінуться нинішні 35 actions

| Група                                                                                                                                    | Скільки | Куди                                                                                                        |
| ---------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------- |
| `markTileDirty` / `markTilesDirty` / `markAllTilesDirty` / `consumeDirtyTileIds`                                                         | 4       | **видалити** — замінює потік подій                                                                          |
| `scheduleLocationRespawn` / `clearLocationRespawn` / `getLocationRespawnRemainingMs` / `isLocationRespawning` / `syncLocationRespawn`    | 5       | session/campaign-шар (сервіс уже є)                                                                         |
| `initFog` / `revealAroundHero` / `revealTile` / `revealEntryTile`                                                                        | 4       | `engine/map/fog-service` (уже є), застосовується через події                                                |
| `bootstrapWorld` / `goToLocation` / `leaveCurrentLocation` / `loadLocationMap`                                                           | 4       | `open()` + `location-navigator`                                                                             |
| `placeHeroAtEntry` / `placeHeroAtCampfire` / `placeHeroOnArrival`                                                                        | 3       | `engine` spawn-placement + команда                                                                          |
| `loadFromStorage` / `saveToStorage` / `clearStoredLocation` / `forgetStoredLocationPosition` / `clearAllWorlds` / `enableCombatAutosave` | 6       | `WorldRepository`                                                                                           |
| `startWorldLoop` / `stopWorldLoop`                                                                                                       | 2       | session-шар                                                                                                 |
| `buildEngineContext` / `hydrate`                                                                                                         | 2       | composition root                                                                                            |
| `executeHexAction`                                                                                                                       | 1       | `dispatch(START_HEX_ACTION)`                                                                                |
| `hydrateResourcesFromConfig` / `hasGraveMarker`                                                                                          | 2       | `engine` (обидва вже частково є)                                                                            |
| `respawnHeroAtCamping`                                                                                                                   | 1       | сага/use-case сценарію — торкається бою, героя, роутера й persistence, тобто не є дією стора за визначенням |
| `getTileAt`                                                                                                                              | 1       | гетер                                                                                                       |

**35 → 4 методи + 3 гетери.**

### 6.8. Як цим користуватись

Не робити з цього «фазу G7» і не намагатися дійти сюди одним заходом — при поточному fan-in це той самий big-bang, від якого застерігає 3.4.

Замість цього — два практичні застосування:

1. **Як чекліст на код-рев'ю.** Новий код, що кладе правило, `localStorage`, таймер або посилання на інший стор у `world-map-store`, — відхиляється з посиланням на 6.5.
2. **Як перший конкретний крок — тільки `buildEngineContext` у composition root.** Це найдешевша зміна з найбільшим ефектом (розчиняє більшість з 11 циклів) і вона не потребує чіпати жодного правила гри.

Решта 6.7 виконується **опортуністично**: коли фаза з `GAMEPLAY-VERTICAL-SLICE-PLAN.md` і так торкається відповідної ділянки — вона переїжджає за контрактом, а не окремим рефакторинг-проєктом. Нова механіка (картки, кубики, ініціатива) з першого дня пишеться як команда рушія, а не як action стора; тоді контракт досягається природним чином, а не героїчним зусиллям.

**Рішення (12.08.2026): крок 2 свідомо відкладено, не заплановано окремою сесією.** Зважили виграш/цінu:

- Виграш точніший, ніж написано вище: `buildEngineContext` пояснює **4 з 11** попереджень (`world-map-store`↔`hero-store`, `world-map-store`↔`combat-store`), не «більшість» — решта 7 йдуть через окремий, навігаційний ланцюжок (`router`↔`user-store`↔`location-navigator`↔`hex-world-map.vue`), якого цей крок не торкається.
- Це чисто інфраструктурна робота без гравцевої цінності, а `CLAUDE.md` прямо каже: «Next up — gameplay content, not infra» → `GAMEPLAY-VERTICAL-SLICE-PLAN.md`.
- Половина виграшу — цикл із `combat-store`, а бойовий дизайн ще не усталений (окрема стояча відмітка в `CLAUDE.md`); ця DI-обв'язка може знову зміститись, коли бій таки редизайнять.

Використовуємо документ так, як радить п.1: як чекліст на код-рев'ю. Крок 2 виконуємо опортуністично — коли хтось реально впреться в один із цих 4 циклів під час gameplay-роботи, або коли з'явиться вікно між фазами `GAMEPLAY-VERTICAL-SLICE-PLAN.md`.

**Старт чату (тільки для кроку 2 із 6.8, коли настане нагода):**

```
Working in hexoflat. Read docs/refactoring/GOD-CLASS-REFACTORING-PLAN.md — Part 6, especially
6.5 and 6.6. Do one thing only: move buildEngineContext() out of world-map-store.ts into a
composition-root module that assembles the engine context once at app startup and injects it,
instead of rebuilding it from 5 Pinia stores on every call. Goal is to dissolve the cycles
currently grandfathered in KNOWN_CYCLE_FILES — after the change, remove from that list every
file that no longer needs it and report which ones genuinely still do, with the reason. No
behaviour changes, no new commands, no other extractions. Dedicated branch. Ask before commit.
```
