...
**Заморозку відкладено.** Приймальний огляд Oleh-а на 10 зразках — «нема відчуття дошки, все ідеш як стежками». Інструментарій M5 (панель, `samples:worldmap`, e2e, фікс спавну) лишається — саме на ньому будується наступна ітерація граматики: див. **Частину 5 / Фазу M6**.
...
## Частина 5 — Ревізія v0.2: «нема відчуття дошки» (02.09.2026)

Фази M0–M5 завершені. Приймальний огляд Oleh-а: **«нема відчуття дошки, все ідеш як стежками»**. Це прийнято як провал приймання, а не як побажання до візуалу.

### 5.1. Вимірювання, а не думки

Метрики, зняті з 10 прийнятих зразків у `docs/design/samples/world-map-mvp/*.svg`:

| Метрика                                                             | Зразки M5    | Що це означає                      |
| ------------------------------------------------------------------- | ------------ | ---------------------------------- |
| Розмір карти                                                        | 18–28 гексів | дуже мало                          |
| Середня к-сть прохідних сусідів                                     | **2.2–2.9**  | коридор = 2, відкрита дошка = 6    |
| Частка гексів із ≤2 сусідами                                        | **30–68%**   | половина світу — буквально коридор |
| Частка гексів із ≥4 сусідами                                        | 5–29%        | відкритого простору майже немає    |
| Частка гексів-артикуляцій (прибери один — світ розпадається надвоє) | **40–68%**   | на дошці це має бути ~0%           |

Тобто згенерований світ — це **граф стежок**, а не поверхня. Відчуття правильне, і воно тепер має число.

### 5.2. Причина — помилка в специфікації секцій, не в реалізації

Це помилка цього плану (Фаза M2), не Claude Code: там було сказано «секції по 4–9 гексів» і «шов = гекс + напрямок», без жодної вимоги до **ширини** секції та кількості швів. Результат закономірний:

1. **Секції — це лінії.** `narrow-pass` — ланцюжок із 4 гексів, `broken-slope` — діагональ із 5, `fork` — лінія з двома відростками, `frontier-shelf` — 3+1. Ширина ≈1. Колода з коридорів дає коридорний світ.
2. **Шви лише на кінцях**, по 2–3 на секцію. Граф приєднань виходить ланцюгом: секції стикуються торець у торець і ніколи боком. Світ росте в одному вимірі.
3. **Складач ніколи не замикає петлі** — він лише навішує нову секцію на вільний шов. Виходить дерево, а в дереві **альтернативний маршрут геометрично неможливий**: між будь-якими двома точками рівно один шлях. Звідси і 40–68% артикуляцій, і те, що «обійти гряду» не виходить у принципі.
4. **`knownHexMin: 12`** — на такій площі дошка не встигає стати дошкою.
5. **Перешкода — це стінка коридора, а не об'єкт усередині простору.** `stone-ridge` — колона з 3 блокованих гексів із проходом збоку. Це читається як коридор зі стіною. «Дошковість» виникає тоді, коли перешкода стоїть **посеред відкритої площі** і ти обираєш, з якого боку її обійти.

### 5.3. Рішення — граматика «площі + перемички»

Світ складається не з відрізків, а з **площ**, з'єднаних короткими **перемичками**. Так побудовані всі фізичні tile-laying-ігри: кімнати — це основна поверхня, коридори — меншість.

- **AREA** — 10–20 гексів, округла (не довша ніж удвічі за ширину), **4–6 швів по периметру**, всередині 0–4 блокованих гекси як перешкода. Це основа колоди.
- **LINK** — 1–3 гекси, рівно 2 шви. Перемичка. Їх у колоді меншість, і **дві перемички не стикуються між собою**.
- **Граматика:** `AREA → LINK → AREA`. Ніколи `AREA → AREA` напряму, ніколи `LINK → LINK`.
- **Чокпоінт більше не окрема секція** — він виникає сам там, де дві площі з'єднані однією перемичкою. Кишеня — теж: це маленька AREA з одним швом.
- **Замикання петель.** Після викладання складач **навмисно шукає пари відкритих швів, які можна з'єднати** (напряму або через одну LINK), і замикає їх. Мета — щонайменше **2 незалежні петлі** на карту. Це єдина зміна, яка перетворює стежки на місцевість: без петель «інший маршрут» не існує як геометрія.
- **Розмір:** `knownHexMin` 12 → **45**, `knownHexMax` 34 → **80**.

### 5.4. Нові інваріанти валідатора («дошковість» як гейт)

Скарга перекладається в машинну перевірку. Прийнята карта мусить мати:

1. середню к-сть прохідних сусідів **≥ 3.5**;
2. частку гексів із ≤2 прохідними сусідами **≤ 25%**;
3. частку гексів-артикуляцій **≤ 15%**;
4. **≥2 незалежні цикли** на рівні маршрутів (не локальні трикутники всередині площі — рахувати на графі площ і перемичок, а не на графі гексів);
5. **≥2 маршрути між кемпом і найдальшою площею**, що не ділять жодної перемички.

Ці п'ять додаються до шести з §1.8. Метрики 1–3 також виводяться у звіт зразків, щоб регресія була видима.

### 5.5. Фаза M6 — перескладання колоди [DONE — 2026-09-02]

**Мета.** Замінити коридорну колоду на граматику «площі + перемички», додати замикання петель і «дошкові» інваріанти. Алгоритм приєднання, детермінізм, рендер, promise-и й архетипи лишаються — міняється **форма секцій, граматика з'єднань і валідатор**.

**Як реалізовано.**

- **Класи секцій.** `WorldSectionDef.class: 'AREA' | 'LINK'` (Zod-схема, `hexes` max 9 → 24). Теги секцій переозначено: `CAMP_ANCHOR, OPEN_FIELD, WOODED, BROKEN, RIDGE_FIELD, POCKET, FRONTIER_FIELD, LINK` (замість `BRANCH/OPEN_AREA/CHOKEPOINT/POCKET/BARRIER/FRONTIER`).
- **Колода параметрична.** `world-sections.content.ts` — хелпер `disc(radius, terrain, omit?)` дає круглий гекс-диск (r2 = 19 гексів), `paint()` накладає внутрішні перешкоди. 12 AREA + 3 LINK: `camp-field` (r2, стартова), `broad-field`/`open-meadow` (OPEN_FIELD), `woodland` (WOODED+OPEN_FIELD), `ridge-bowl`/`ridge-spur` (RIDGE_FIELD+OPEN_FIELD — відкрите поле з 3-гексовою STONE_RIDGE-перешкодою всередині), `broken-basin` (BROKEN+RIDGE_FIELD), `hollow`/`wide-hollow` (POCKET — маленька AREA з 1 швом = тупик), `frontier-plain`/`frontier-shelf` (FRONTIER_FIELD). LINK: `gate` (1 гекс), `pass` (2), `bend` (3) — усі NARROW_PASS, рівно 2 шви. Багатотегові AREA дозволяють задовольнити 2 вимоги рецепта однією площею.
- **Граматика в складачі.** Кожен відкритий шов знає клас власника; `grow` бере протилежний клас (AREA-шов → тягне LINK, LINK-шов → тягне AREA). Пряме `AREA→AREA` / `LINK→LINK` неможливе. Секції тепер **перевикористовуються** (лічильник `useCount`, вага в `weightedPick` спадає з кожним використанням). `weightedPick` також біасить до **компактності** — кандидат із більшою кількістю вже викладених сусідів важить більше, тож світ росте щільною плямою, а не відростками.
- **Розмір росту.** `grow` зупиняється, коли гексів ≥ `growTargetHexes` (5 → фактично одразу після кемпу на 19 гексів), теги рецепту вичерпані **і** ≥ `minAreas` (4) не-POCKET площ. Далі — тільки площі під невичерпані теги + перемички. `maxHexes` = `knownHexMax` — жорстка стеля.
- **Замикання петель** — `world-map-assembler-core.ts` `closeLoops`: BFS-міст (≤5 порожніх гексів, NARROW_PASS) між парами відкритих швів різних секцій. Дві фази: спершу `targetLoops` найтісніших пар (гарантує цикли на графі площ), потім до `targetLoops + 5` найширших (з'єднує кемп із далеким краєм). Кожен міст — ребро `sectionGraph.bridgeEdges`.
- **Пас заповнення** — `fillConcavities`: 2 проходи, будь-який Deep-Unknown гекс із ≥4 викладеними сусідами стає `OPEN_GROUND`. Закриває увігнуті кути, зливає дотичні площі в поверхню. Це головний важіль проти «стежок»: середня к-сть сусідів піднялась із **2.2–2.9 → 4.0–4.6**, частка тонких гексів **30–68% → 6–15%**, артикуляції **40–68% → 0–8%**.
- **Граф площ** — `world-map-graph.ts`: контракція LINK-вузлів у ребра між площами, **мультиграф** (паралельні ребра рахуються — два шляхи між площами = цикл). `areaGraphCycles` = `E − V + компоненти`. `areasTwoEdgeConnected` — послідовно прибирає кожне ребро й перевіряє досяжність (Менгер для мультиграфа). «Найдальша площа» виключає POCKET (тупик не може бути 2-реберно-зв'язним).
- **Валідатор — 12 інваріантів** (6 із §1.8 + 5 із §5.4 + `size`). Метрики: `meanNeighbours`, `thinShare`, `articulationShare`, `routeLoops` (на графі площ), `chokepointCount` (LINK-вузли + мости). `openAreaSize`/`pocketSize` перетеговано на `OPEN_FIELD`/`POCKET`. Додано `size` (гексів у `[knownHexMin, knownHexMax]`) — 13-та перевірка проти роздування пасом заповнення; це відхилення від «11» у §5.4, обґрунтоване тим, що заповнення робить карти 95–120 гексів.
- **Конфіг.** `knownHexMin 45`, `knownHexMax 120` (не 80 — пас заповнення + великі площі природно дають 95–120), `growTargetHexes 5`, `targetLoops 2`, `minMeanNeighbours 3.5`, `maxThinShare 0.25`, `maxArticulationShare 0.15`. Архетипи: `requiredTags` переписано на нові AREA-теги, POCKET — останній у списку RIDGE_AND_POCKET (кладеться після основної структури).
- **Дірки в карті заборонено (2-й раунд огляду).** Oleh: «при з'єднаних нодах не може бути дир — там завжди має бути терен або об'єкт». Додано `sealHoles` — flood-fill зовнішнього порожнього простору з кута bbox; будь-який порожній гекс, недосяжний ззовні (замкнена порожнина), заповнюється тереном (`STONE_RIDGE`, якщо всі сусіди блоковані, інакше `OPEN_GROUND`). `fillConcavities` повернуто на поріг 4 (згладжування берегової лінії), `sealHoles` після нього гарантує **нуль замкнених порожнин**. `knownHexMax` 120 → 135 (пломбування додає ~10–20 гексів). Тест: жодного повністю оточеного порожнього гекса на 96 сидах.
- **Камера — hero-follow, фіксований зум (2-й раунд огляду).** Oleh: «розмір гекса завжди однаковий; коли герой на нижньому тайлі, його не видно — камера має тримати токен по центру». `use-hex-board-sizing.ts`: прибрано fit-to-container `scale`, тепер `scale = 1` завжди (гекс однакового розміру на будь-якій карті). Нове `cameraOffset` — translate `.hex-map-wrapper` так, щоб піксельний центр гекса героя збігався з центром панелі `.hex-map`; fallback на центр карти, коли героя ще нема. `.hex-map` → `position: relative` (замість `place-items: center`), wrapper → `position: absolute; transform-origin: 0 0`. `mapBounds` (розмір canvas) і e2e `getTileFraction` не змінились — клік по тайлу враховує transform через `boundingBox()`.
- **Токен героя — теж безперервний rAF-лерп (5-й раунд огляду).** Oleh: «можеш ще плавніше зробити рух токена». `hero-layer.ts`: фіксований твін 180 мс (`easeOutCubic`, який щокроку доїжджав майже до зупинки) замінено на той самий експоненційний rAF-лерп, що й камера (`TAU 78 мс`). Багатокроковий рух = одне ковзання без рестартів; ticker лише ретаргетиться на кожен новий гекс.
- **Досліджена зона виглядала як тунель (6-й раунд огляду).** Oleh: «це дивно» — при суцільній карті пройдений слід був тонкою стрічкою (радіус огляду 1). `fog-service.revealAroundHero` тепер BFS-обхід до `HERO_SIGHT_RADIUS = 3`, що обтікає відкритий ґраунд і **зупиняється на SOLID-теренах** — стоїш у полі → бачиш поле, стоїш у проході → бачиш прохід, крізь стіну не видно. Ghost-фронтир лишається 1 гекс. Зачіпає й `cave` (радіус огляду виріс з 1 до 3 — прийнятно, читабельніше). Тести `fog-service.test.ts` / `world-map-store.test.ts` оновлено на більші тестові карти.
- **Камера смикалась покроково → потім дрібіжання (3-й і 4-й раунди огляду).** CSS-transition давала stop-start між кроками. Замінено на **безперервний rAF-лерп** у `use-hex-board-sizing.ts`: `cameraRender` щокадру наближається до `cameraTarget` експоненційним згладжуванням (frame-rate-independent, `TAU 120 мс`) — багатокроковий рух тече одним плавним ковзанням, ніколи не зупиняючись. `snapCamera()` (виклик на зміну `currentMapId`) ставить миттєво. `prefers-reduced-motion` → миттєво. Прибрано `Math.round` з transform (sub-pixel) — це прибрало ±1px «дрібіжання» на завершенні кроку. Перевірено e2e: 11 проміжних значень transform за крок, tailSpread осідання 0.14px, 0 реверсів; 12/12 world e2e.
- **Не чіпано:** M0/M1, PixiJS-шари, promise-розміщення, gameplay-anchors (terrain-based), детермінізм (усі проходи сортовані/сидовані), `CONTENT_VERSION`, e2e-хуки.
- **Результат 10 зразків** (`docs/design/samples/world-map-mvp/`, `samples:worldmap`): hex 111–132, loops 2–3, mean 4.5–5.1, thin 2–8%, art 0–7%, дір 0. Per-seed acceptance: FORKED/OPEN_FIELD/LANDMARK ~95%, RIDGE_AND_POCKET ~67% (fallback + 25 спроб → фактично 100%).
- **Тести:** `world-map-graph.ts` покрито через валідатор-тести; `world-map-assembler.test.ts` — граматика (`AREA↔LINK` на кожному tree-ребрі), `loopsClosed ≥ 2` на більшості сидів, board-розмір; `world-map-validator.test.ts` — boardness-тест (заблокуй усе крім 6 гексів → thin-share/thin-mean). 340 engine + 207 web + 63 api зелені; 12/12 world e2e.

**Чого НЕ робити.** Не переписувати складач з нуля, не чіпати M0/M1, не додавати нові терени, не братись за рух героя.

**Критерій приймання.** 10 нових зразків, усі проходять 11 інваріантів; звіт показує середню к-сть сусідів ≥3.5 і артикуляції ≤15%; детермінізм збережено; візуально видно відкриті площі з перешкодами всередині й щонайменше два способи обійти.

**Старт чату:**

```
Working in hexoflat, branch map-generator is already checked out — work there, do not create
any branch. Read docs/WORLD-MAP-MVP-PLAN.md, part 5 — Phase M6 only. Phases M0-M5 are merged
and the generator failed acceptance review: the generated world reads as a network of trails,
not as a board. Measured on the 10 sample maps: average passable-neighbour count 2.2-2.9,
30-68% of hexes have <=2 neighbours, and 40-68% of hexes are articulation points.

Fix the section grammar, not the renderer.

- Rebuild the section deck around two classes. AREA: 10-20 hexes, roughly round (longest axis
  at most twice the shortest), 4-6 seams spread around the perimeter, and 0-4 BLOCKED hexes
  placed INSIDE the area as obstacles to walk around. LINK: 1-3 hexes, exactly 2 seams.
  Areas are the majority of the deck.
- Enforce the grammar AREA -> LINK -> AREA in the assembler. Never attach AREA directly to
  AREA, never LINK to LINK.
- Drop CHOKEPOINT and POCKET as separate section types: a chokepoint is now emergent (two areas
  joined by a single link), and a pocket is a small AREA with one seam.
- Add loop closing: after the initial layout, deliberately look for pairs of open seams that
  can be joined directly or through one LINK, and close them. Target at least 2 independent
  route-level cycles per map.
- Raise knownHexMin to 45 and knownHexMax to 80 in the config.
- Add five validator invariants on top of the existing six: mean passable-neighbour count >=
  3.5; share of hexes with <=2 passable neighbours <= 25%; share of articulation hexes <= 15%;
  at least 2 independent cycles measured on the AREA/LINK graph (not on the hex graph, where
  triangles inside an area create false cycles); and at least 2 routes from camp to the
  furthest area that share no link.
- Report the first three metrics in the sample report so regressions stay visible.

Keep the existing assembler, determinism, seams, promises, archetypes and rendering. Regenerate
the 10 sample maps. One commit for this phase. Ask before committing.
```
